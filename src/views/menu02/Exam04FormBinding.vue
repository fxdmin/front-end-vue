    ...
    <div class="card-body">
      <form v-on:submit.prevent="handleSubmit()">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-10">
            <input type="radio" class="form-check" v-model="product.sex" value="man"/>
            <label class="form-check-label">Man</label>

            <input type="radio" class="form-check" v-model="product.sex" value="woman"/>
            <label class="form-check-label">Woman</label>
          </div> 
        </div>

        <div>
          <button class="btn btn-dark btn-sm mr-2" v-bind:disabled="disabledRegBtn">등록</button>
          <router-link to="/" class="btn btn-outline-dark btn-sm">취소</router-link>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  computed:{
    disabledRegBtn(){
      //name이나 company둘 중 하나라도 비어있으면 true 아니면 false
      //true라는 것은 비활성화하겠다는 것
